...
def home():
    ...
    if request.method == 'POST':
        ...
        # Use get method to safely retrieve the value of 'user_input'
        ...
        if user_input is not None:
            ...
            # If you want to return JSON data for the POST request
            return jsonify(response(user_input))

        # If 'user_input' is not present in the form data
        return jsonify({'status': 'error', 'message': 'KeyError: user_input not found in form data'})

    # If you want to return JSON data for the GET request
...
def response(user_input: str, force_generate_new: bool = False, previous_response: relational_db.Response  = None):
    ...
    return {
        ...
        'answer_message': {
            ...
            'saved_question': answer_message.saved_question,
            'message': answer_message.message,
            'sources': [source.to_dict() if hasattr(source, 'to_dict') else source for source in answer_message.sources],

            #[source.to_dict() for source in answer_message.sources],
            'comment': answer_message.comment
        },
        'time_intervals': context["time_intervals"]
    }
...
def retrieve_relevant_response(answer_message: message.Answer, user_input: str, relevant_q: dict, distance: float) -> message.Answer:
    ...
    with app.app_context():
        ...
        # Check if relevant_response is None
        if relevant_response is None:
            # Handle the case where there is no relevant response
            # You might want to perform some alternative actions here
            # For now, just print a message and return the original answer_message
